...
def benchmkark(df, sp, pesos=list):    
    ...
    st.write('-' * 100)
    st.text('Si hubieses invertido $100000: ')
    st.text(f"Portfaolio: ${rdo_last.Value}")
...
st.number_input('Ingrese el numero de activos:', min_value=0, max_value=5, key='N')
if ('N' in st.session_state) and (st.session_state.N > 1):
        ...
        if st.session_state.N == len(tickers_list):
            ...
            if option == 'Simulacion de Monte Carlo':
                ...
                st.subheader("Benchmark")
                lista_pesos = st.selectbox('Seleccione los pesos para cada activos:', result.Pesos)
                bench = benchmkark(df, sp, lista_pesos)
                st.write(bench)
                st.subheader('Benchmark, Valor del portafolio vs SP500')
                st.line_chart(bench[['Value', 'SP500']])

            elif option == 'Optimizacion por volatibilidad':
                n = st.session_state.N
                df_retornos = st.session_state.ret_log
            ...
            st.write("Debe escribrir la misma cantidad de tickers que configuro previamente")
    
else:
    st.warning('Confgure la cantidad de activos y sus nombres.', icon="⚠️")
